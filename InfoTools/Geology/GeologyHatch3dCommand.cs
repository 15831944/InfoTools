...
namespace Civil3DInfoTools.Geology
{
    ...
    //Полигон делится на отдельные полигоны по точкам пересечения (при этом учитываются участки полного перекрытия (overlap)
    //- участки перерытия не должны попадать в результат)
    //...

    ...
    public class SetIds : AddInPlugin
    {
        ...
        public static void SetIdToItem(ComApi.InwOpState3 oState, ModelItem item)
        {
            ...
            foreach (ComApi.InwGUIAttribute2 attr in propertyNode.GUIAttributes())
            {
                if (attr.UserDefined)
                {
                    if (attr.ClassUserName.Equals(IdDataTabDisplayName))
                    {
                        indexToSet = i;
                        ...
                        {
                            if (prop.UserName.Equals(IdPropDisplayName) /*&& prop.value is string && !String.IsNullOrWhiteSpace(prop.value)*/)
                            {
                                //Панель свойств содержит Id
                                needToOverwriteId = false;
                            }
                            else if (prop.UserName.Equals(MaterialIdPropDisplayName))
                            {
            ...
            {
                propsToSet.Properties().Add(Utils.CreateNewUserProp(oState, IdPropDisplayName, Guid.NewGuid().ToString()));
                idCount++;
            }
            if (needToOverwriteMaterialId)
            {
                propsToSet.Properties().Add(Utils.CreateNewUserProp(oState, MaterialIdPropDisplayName, "_"));
            }

            if (needToOverwriteId || needToOverwriteMaterialId)
            {
                propertyNode.SetUserDefined(indexToSet, IdDataTabDisplayName, "S1NF0", propsToSet);
            }
        }
    }
}
